...
TEST_F(ArionTest, MultipleEmulations)
{
    ...
    for(uint8_t i = 0; i < 3; i++) {
        ...
        std::shared_ptr<Arion> arion = Arion::new_instance({this->targets_path + "/simple_print/build/simple_print"}, "/", {},
                                                       std::filesystem::current_path(), std::move(config));
