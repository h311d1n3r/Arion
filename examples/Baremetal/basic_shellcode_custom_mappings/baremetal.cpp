...
int main()
{
    ...
    std::map<CPU_ARCH, ShellcodeInfo> shellcodes = {
        {CPU_ARCH::X86_ARCH,   {shellcode_x86,   sizeof(shellcode_x86),   32}},
        {CPU_ARCH::X8664_ARCH, {shellcode_x8664, sizeof(shellcode_x8664), 64}},
        {CPU_ARCH::ARM_ARCH,   {shellcode_arm,   sizeof(shellcode_arm),   32}},
        {CPU_ARCH::ARM64_ARCH, {shellcode_arm64, sizeof(shellcode_arm64), 64}}
    };

    for (const auto& [arch, info] : shellcodes) {
        ...
        baremetal->arch = arch;
        baremetal->bitsize = info.bit_size;
        auto coderaw = baremetal->coderaw;
        
        coderaw->insert(coderaw->end(), info.data, info.data + info.size);
