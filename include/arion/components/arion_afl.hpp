...
using ARION_AFL_CRASH_CALLBACK = std::function<bool(std::shared_ptr<Arion> arion, uc_err res, char* input, int input_len, int persistent_round, void *user_data)>;
...
struct ARION_AFL_PARAM
{
    ...
    void *user_data;
    ARION_AFL_PARAM(std::weak_ptr<Arion> arion, std::shared_ptr<ARION_CONTEXT> ctxt, ARION_AFL_INPUT_CALLBACK input_callback, ARION_AFL_CRASH_CALLBACK crash_callback, void *user_data)
        : arion(arion), ctxt(ctxt), input_callback(input_callback), crash_callback(crash_callback), user_data(user_data) {};
};
...
public:
    ...
    void ARION_EXPORT fuzz(std::string input_f, ARION_AFL_INPUT_CALLBACK input_callback, ARION_AFL_CRASH_CALLBACK crash_callback, std::vector<arion::ADDR> exits, std::vector<int> signals = {SIGSEGV, SIGABRT}, bool always_validate=false, uint32_t persistent_iters=1000, void* user_data=nullptr);
};
