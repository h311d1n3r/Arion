...
struct SYSCALL_FUNC
{
    ...
    SYSCALL_FUNC(
                 std::function<uint64_t(std::shared_ptr<Arion> arion, std::vector<SYS_PARAM> params)> func, std::vector<std::shared_ptr<ArionType>> signature)
...
class ARION_EXPORT LinuxSyscallManager
{
  private:
    ...
    std::shared_ptr<SYSCALL_FUNC> make_sys_func(std::function<uint64_t(std::shared_ptr<Arion> arion, std::vector<SYS_PARAM> params)> func, SignatureArgs&&... signature) {
    ...
    void print_syscall(std::shared_ptr<Arion> arion, std::string sys_name, std::vector<std::shared_ptr<ArionType>>  signature, std::vector<SYS_PARAM> func_params, uint64_t syscall_ret);
